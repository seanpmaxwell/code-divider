import type { InitialSettings } from '../types/settings';

// ========================================================================= //
//                                 CONSTANTS                                 //
// ========================================================================= //

// Each language declares its file extensions, the comment syntax markers are
// written in (open/close, close empty for line comments), and the bookends
// used for the generated header lines (defaults to the comment syntax). The
// marker regexes are built from these — no regexes in config files.
const DefaultConfig = {
  filter: {
    include: [],
    // `**/` makes these apply at any depth (e.g. nested `node_modules` in a
    // monorepo). `bin`, `lib` and `dist` stay top-level only, since folders
    // with those names are often real source deeper down (e.g. `src/lib`).
    exclude: [
      'bin',
      'lib',
      'dist',
      '**/node_modules',
      '**/*.log',
      '**/*.json',
      '**/.vscode',
      '**/.idea',
      '**/.claude',
    ],
  },
  All: {
    CharacterLimit: 79,
    FillerCharacter: '=',
    RegionLabelFormat: 'uppercase',
    SectionLabelFormat: 'capitalize',
  },
  JavaScript: {
    Extensions: ['ts', 'tsx', 'js', 'jsx', 'mjs', 'cjs'],
    Comment: ['// ', ''],
    Bookends: ['// ', ' //'],
  },
  Java: {
    Extensions: ['java'],
    Comment: ['// ', ''],
    Bookends: ['// ', ' //'],
  },
  Css: {
    Extensions: ['css', 'scss'],
    Comment: ['/* ', ' */'],
    Bookends: ['/* ', ' */'],
  },
  C: {
    Extensions: ['c', 'h'],
    Comment: ['// ', ''],
    Bookends: ['// ', ' //'],
  },
  Cpp: {
    Extensions: ['cpp', 'cc', 'cxx', 'hpp', 'hh', 'hxx'],
    Comment: ['// ', ''],
    Bookends: ['// ', ' //'],
  },
  Go: {
    Extensions: ['go'],
    Comment: ['// ', ''],
    Bookends: ['// ', ' //'],
  },
  Rust: {
    Extensions: ['rs'],
    Comment: ['// ', ''],
    Bookends: ['// ', ' //'],
  },
  Php: {
    Extensions: ['php'],
    Comment: ['// ', ''],
    Bookends: ['// ', ' //'],
  },
  Ruby: {
    Extensions: ['rb'],
    Comment: ['# ', ''],
    Bookends: ['# ', ' #'],
  },
  Sql: {
    Extensions: ['sql'],
    Comment: ['-- ', ''],
    Bookends: ['-- ', ' --'],
  },
  Bash: {
    Extensions: ['sh'],
    Comment: ['# ', ''],
    Bookends: ['# ', ' #'],
  },
  Python: {
    Extensions: ['py', 'pyi', 'pyw'],
    Comment: ['# ', ''],
    Bookends: ['# ', ' #'],
  },
} as const satisfies InitialSettings;

// ========================================================================= //
//                                  EXPORT                                   //
// ========================================================================= //

export default DefaultConfig;
