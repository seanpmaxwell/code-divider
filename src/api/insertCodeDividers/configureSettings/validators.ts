...
export function validateFilterSettings(filter: unknown): FilterSettings {
  ...
  if (!isStrArr(include)) {
    throw new Error(
      `invalid ${CONFIG_FILE_NAME}: "filter.include" must be an array of glob patterns, e.g. ["src"]`,
    );
  }
  if (!isStrArr(exclude)) {
    throw new Error(
      `invalid ${CONFIG_FILE_NAME}: "filter.exclude" must be an array of glob patterns, e.g. ["node_modules"]`,
    );
  }
...
export function validateLangSpecificSettings(
  ...
  if (!isStrArr(bookends) || bookends.length !== 2) {
    throw new Error(
      `invalid ${CONFIG_FILE_NAME}: "${lang}" Bookends must be of type [string, string]`,
    );
  }
  ...
  if (!isStrArr(Extensions)) {
    throw new Error(
      `invalid ${CONFIG_FILE_NAME}: "${lang}" Extensions must be of type string[]`,
    );
  }
