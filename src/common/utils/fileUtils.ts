 ...
 */
function createItem(relativePath: string, startingDir?: string): void {
 ...
 */
function rmItem(relativePath: string, startingDir?: string): void {
  let finalPath = relativePath;
  if (startingDir && !path.isAbsolute(relativePath)) {
    finalPath = path.join(startingDir, relativePath);
  }
 ...
 */
function listDirItemsShallow(targetPath: string): string[] {
 ...
 */
function filterDirItems(
  include: string[] = [],
  exclude: string[] = [],
  targetPath: string,
): string[] {
  let items = listDirItemsDeep(targetPath);
  if (include.length > 0) {
